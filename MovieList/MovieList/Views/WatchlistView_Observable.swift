//
//  WatchlistView_Observable.swift
//  MovieList
//
...
// WatchlistView_Observable showing how @Observable changes the view side.
struct WatchlistView_Observable: View {
    
    // @State replaces @StateObject when using @Observable
    // SwiftUI stores the instance outside the struct (same as before)
    // @Observable macro handles change tracking (replaces @Published + objectWillChange)
    @State private var viewModel = WatchlistViewModel_Observable()
        ...
        VStack(spacing: 0) {
            ...
            List {
                ForEach(filteredMovies) { movie in
                    NavigationLink(
                        destination: MovieDetailView_Observable(
                            movie: movie,
                            viewModel: viewModel
                        )
                    ) {
